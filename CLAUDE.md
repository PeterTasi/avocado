...
| POST | `/api/courses/{id}/activate` | 切回既有課程（概念／圖譜／複習全部 scope 到 active course） |
...
- **待辦 L2 已修**（commit `a533a74`，8/18）：空白頁不再被誤判成 OCR 成功——回吐的課程名／頁標是我們自己塞進 prompt 的已知字串，用精確比對（扣掉這兩個值＋標點空白後殘留是否為空）判定，不是啟發式門檻。
...
| L2 | ✅ 已修（8/18）| 空白頁被算成 OCR 成功。詳見 DEVLOG |
