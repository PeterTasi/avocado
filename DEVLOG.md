...
中原大學 2026「AI 教與學標竿課程實踐暨成果展演競賽」學生組決賽，組名 avocado。

簡報 `docs/AdaptLearn-決賽簡報.pdf`（正片 12 頁 + 附錄 8 頁），
講稿 `docs/pitch-final.md`，備詢小抄 `docs/AdaptLearn-評審問答.pdf`。

### 評審實際問了什麼

| # | 問題 | 小抄有沒有涵蓋 |
|---|---|---|
| 1 | 節點概念是怎麼呈現出來的？**先修關係怎麼建立**？ | A9 有，但只寫了「三道限制」，**沒寫抽取流程** |
| 2 | 跨課程連結那邊 —— **上傳好幾門課，技能樹會不會很雜亂**？ | ❌ 沒有這題 |

### 第 2 題是誤會，但誤會本身就是回饋

評審以為多門課的概念會全部畫在同一棵技能樹上。**實際上不會**：

`src/adaptlearn/pipeline.py:546` `get_graphviz()` 用 `active_course_id`
同時過濾 `list_concepts()` 與 `list_edges()` —— 技能樹一次只顯示切中的那一門課；
跨課程連結是技能樹下方的獨立面板（`CrossCourseBridgePanel`），不畫進樹裡。

**產品洞察：** 評審會誤會，代表介面沒把「一次只顯示一門課」講清楚。
待辦：技能樹上方加明顯狀態列「目前顯示：<課程名>（共 N 門課）」。

### 第 1 題完整答案（補進小抄 A9）

`src/adaptlearn/knowledge_graph.py`：

1. LLM 讀教材 → 吐出概念記錄（名稱／章節／說明／**先修清單**）
2. **兩段式過濾**（`_records_to_concepts`）：第一段先收集所有保留的概念名，
   第二段才過濾先修 —— 原本單段過濾會把「先修指向**後面才出現**的概念」那些邊**靜默丟掉**
3. `_build_edges` 把先修清單轉成 `relation="prerequisite"` 的邊
4. LLM 完全沒給先修時，才退回 `_infer_progression_edges` 用章節順序保守推斷

第 2 步跟簡報第 11 頁的誠實原則是同一個故事：**靜默失敗最危險。**

### 小抄後續更新

補上 A9 的抽取流程、新增 A11（多門課技能樹會不會亂），
決賽實際被問到的題目標上「★ 決賽實問」。
...
## 2026-09-02 — 補作答資料讓 demo 畫面有內容（未改程式）

**做法：** 用真實的 `POST /api/questions/{id}/grade` 作答，Gemini 真的批改，答案品質刻意混合（全對／半對／寫進真實誤區），
讓掌握度矩陣有紅黃綠層次而不是一片 0%。腳本會偵測 `_heuristic_grade` 的兩句固定回饋，一降級就中止。

**刻意不碰的四個概念**（簡報第 6 頁卡關歸因的命脈，`graphUtils.traceWeakestUpstream` 的輸入）：
`正交基底` 維持 3 次／66.7%；`內積`、`正交向量`、`正交集合` 維持 0 次作答。
驗證：`/api/mastery/concepts` 事後查證四者數值與事前完全相同，回溯鏈 `內積 → 正交向量 → 正交集合 → 正交基底` 未變。

**結果：** 作答數 7 → 29。手寫課從全 0 變成 伴隨算子 86.7%／埃爾米特矩陣 86.7%／對稱矩陣 75.0%（各 6 次作答）；
PCA 課 特徵分解 100%（3 次）、主成分分析 100%。快照 `demo-rich`（事前快照 `pre-quiz-fill` 可回滾）。

**⚠️ 這批資料的定位：** 是 9/2 為了 demo 補做的，不是使用者一個學期的學習紀錄。
可以說「系統裡有 29 筆真實批改紀錄」，**不能**說「這是我一學期的使用軌跡」。
唯一能稱作使用者本人學習紀錄的仍是 8/18 那 3 筆（正交基底 67%）。

### 三個順帶查到的 demo 風險（都還沒修）

1. **Gemini 免費配額每天 20 次，今天已用完**（log 明確回 `RESOURCE_EXHAUSTED`，`gemini-flash-latest`／`3.7-flash`／`3.5-flash` 全部退化）。
   配額用完時批改會**靜默降級成關鍵字比對**（`_heuristic_grade`），分數是假的但畫面看不出來。
   決賽當天**不要現場按「產生題目」或讓評審作答**，除非確認配額還有。第 22 題那筆降級分數已從 DB 刪除。
2. **`webapp/main.py` 的 index.html 沒有 `Cache-Control: no-cache`**（只有 ETag／Last-Modified）。
   實測瀏覽器會沿用舊的 index.html → 指向 `npm run build` 已刪掉的舊 bundle → 畫面停在改版前的樣子。
   今天驗收時就中招一次，強制重載才恢復。**上台前用無痕視窗或強制重新整理開一次。**
3. **9 題英文模板題**（`Define ...` / `Provide a counterexample ...` / `Given a typical problem ...`）留在「線性代數的正交性」課，
   分佈在 正交矩陣／正交集合／規範正交基底 各 3 題。是 LLM 降級時產生的（`fable5.1-review` #7）。
   評審在測驗頁翻頁有機會翻到，81 題中佔 9 題。
