...
## 待辦 M：重新整理就失憶——兩個 session 閘門讓 demo 畫面歸零（2026-08-18 實機動線驗證挖出）

> **發現方式：** 8/18 跑完整 demo 動線時實機發現。兩者都不是資料問題——資料庫是對的，
> 是前端的 session 狀態把正確的資料擋在外面。

### 問題①：首頁在重新整理後全部顯示 0

- **證據：** `/api/health` 回 `concept_count: 13, attempt_count: 3, accuracy: 0.667`，
  但首頁三張卡片顯示 `0 / 0% / 0`。重新整理後依然如此。
- **根因：** `App.tsx:148` 的 `sessionUploaded` 是 `useState(false)`，**重新整理就重置**。
  它同時擋住 `App.tsx:167` 的概念清單、181 的今晚焦點、195–200 的所有首頁數字。
  必須先進教材頁點一次課程才會打開。
- **為什麼會這樣設計：** 這是 Bug 5 方案 A 的閘門——單租戶全域架構下，
  新訪客不該一進站就看到前一位使用者的資料。**意圖是對的，副作用才是問題。**

### 問題②：測驗頁永遠是空的，逼使用者按下會吃配額的按鈕

- **證據：** 資料庫有 21 題、`/api/questions` 正常回傳，但測驗頁顯示「尚未產生題目」。
- **根因：** `App.tsx:143` 的 `questions` 是 `useState<Question[]>([])`，
  **從不從 `/api/questions` 載入**。`useQuestions()` hook 早就存在（`useApi.ts:204`）卻沒人用。
- **後果（這才是重點）：** 要在畫面上看到任何題目，就**必須按「產生題目」打 Gemini**——
  而免費額度是每模型每天 20 次，且實測會遇到 503。
  **demo 最不該依賴即時 API 的一頁，偏偏是唯一強制依賴的一頁。**

### 方案

1. **`sessionUploaded` 改存 `sessionStorage`。**
   這個 flag 的名字就叫 session，而 `sessionStorage` 的語意剛好完全吻合：
   **重新整理保留、關掉分頁或換人開就清空**。Bug 5 方案 A 的原始意圖完全保留，
   只修掉「重新整理就失憶」。用 try/catch 包住（無痕模式可能拋錯），
   失敗就退回原本的記憶體行為，不讓它炸掉頁面。
2. **測驗題目用既有的 `useQuestions()` 種子載入**，在 `questions.length === 0` 時灌進 state。
   **同樣受 `sessionUploaded` 閘門控制**，與概念清單（167 行）行為一致，
   避免新訪客看到別人的題目。
3. **後端 `list_questions` 補上 active course 過濾。**
   `pipeline.py` 其他 `list_*` 全都有 `active_course_id` scoping，只有它沒有
   （`database.py:538` 是 `ORDER BY created_at DESC` 撈全域）。
   不補的話步驟 2 會引進新問題：切到 A 課卻看到 B 課的題目。
   屬於**待辦 G 同一家族**的潛在 bug——目前沒炸只是因為現在只有一門課有題目（21 題全屬線代正交性）。

### 風險

- `sessionStorage` 在無痕模式／停用儲存時可能拋錯 → try/catch 退回記憶體行為。
- `list_questions` 加過濾是行為變更，但既有呼叫者只有 `/api/questions` 一處、
  且前端目前根本沒用它，破壞面極小。
- **不動**任何吃配額的路徑（出題、批改、概念抽屜）。
- 前端有改 → 需 `npm run build` 並 commit 產物。

### 驗收標準

1. 進站 → 點課程 → **按 F5** → 首頁仍顯示 13／67%／13（目前會變 0）。
2. 重新整理後直接切測驗頁 → 看得到既有題目，且**後端日誌零 Gemini 呼叫**。
3. 切到別門課 → 測驗頁不顯示前一門課的題目。
4. 全測試綠燈、`npm run build` 零錯誤。

### 不在範圍

複習頁顯示的「通過率 73.9% → 98.0%」（待辦 L5 的未驗證佔位公式）現在會出現在評審眼前，
但那是數字可信度問題不是 bug，要處理是另一個決策。
